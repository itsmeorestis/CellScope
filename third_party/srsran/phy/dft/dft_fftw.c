...
#ifdef _WIN32
// CELLSCOPE: MinGW has no lockf(); the FFTW wisdom cache does not need advisory
// locking here (single process), so map it to a no-op.
#define F_LOCK 1
#define F_ULOCK 0
...
#ifdef _WIN32
  // CELLSCOPE_BEGIN ensure wis filename is available.
  if (homedir == NULL) {
