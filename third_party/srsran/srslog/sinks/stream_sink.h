...
/// CELLSCOPE: MinGW defines stdout/stderr as function-call macros, so the
/// enum enumerators cannot share the name. Use out/err as labels instead.
enum class sink_stream_type { out, err };

namespace internal {
inline std::FILE* stream_of(sink_stream_type t) {
  return (t == sink_stream_type::out) ? stdout : stderr;
}
}
...
public:
  stream_sink(sink_stream_type s, std::unique_ptr<log_formatter> f) :
    sink(std::move(f)), handle(internal::stream_of(s))
  {}
