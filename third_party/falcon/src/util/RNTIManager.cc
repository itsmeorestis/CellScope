...
void RNTIManager::addEvergreen(uint16_t rntiStart, uint16_t rntiEnd, uint32_t formatIdx) {
  std::unique_lock<std::recursive_mutex> rntiManagerLock(rntiManagerMutex);
...
bool RNTIManager::validate(uint16_t rnti, uint32_t formatIdx) {
  // CELLSCOPE: lock the whole validation (recursive mutex lets the helpers'
  // own locks re-enter safely). Without this, the histogram reads here raced
  // with concurrent add()/stepTime() writes from other decode workers.
  std::unique_lock<std::recursive_mutex> rntiManagerLock(rntiManagerMutex);
...
void RNTIManager::stepTime() {
  // CELLSCOPE: writes histograms/remainingCandidates/timestamp -> must lock.
  std::unique_lock<std::recursive_mutex> rntiManagerLock(rntiManagerMutex);
