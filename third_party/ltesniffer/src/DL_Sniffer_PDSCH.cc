			...
			if ((decoding_mem.mcs_table == DL_SNIFFER_64QAM_TABLE) || (decoding_mem.mcs_table == DL_SNIFFER_256QAM_TABLE))
			{
				...
				if (mimo_ret == SRSRAN_SUCCESS)
				{
					...
					if (pdsch_cfg->grant.tb[0].enabled || pdsch_cfg->grant.tb[1].enabled)
					{
						...
						// CELLSCOPE: forward equalized constellation symbols for the
						// pinned UE (no-op unless cur_rnti is the constellation target).
						cellscope::emitLteConst(cur_rnti,
												(const float*)falcon_ue_dl->q->pdsch.d[0],
...
void PDSCH_Decoder::print_api_dl(uint32_t tti, uint16_t rnti, int id, std::string value, int msg)
{
	...
	std::cout << std::endl;
	// CELLSCOPE: forward the decoded identity/message to the LTE engine.
	cellscope::emitLteEvent(rnti, id_name.c_str(), value.c_str(), msg_name.c_str());
}